...
import { zoneSchema, siteSchema } from "@/lib/validation/admin";
...
export async function saveSiteAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  return runAction(async () => {
    ...
    revalidatePath("/admin/sites");
    // Tras crear la sede se abre su ficha para registrar las zonas.
    return { ok: true, message: "Sede creada. Ahora puedes agregar sus zonas.", redirectTo: `/admin/sites/${site.id}` };
  });
}
...
export async function saveZoneAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  return runAction(async () => {
    const ctx = await serviceContext(await requirePermission("sites.manage"));
    const input = parseForm(zoneSchema, formData);
    await sitesService.saveZone(input, ctx);
    revalidatePath(`/admin/sites/${input.siteId}`);
    return { ok: true, message: input.id ? "Zona actualizada." : "Zona creada." };
  });
}
