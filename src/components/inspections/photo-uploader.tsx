...
import { deleteEvidenceAction } from "@/server/actions/inspections.actions";
...
type Target = { kind: "answer"; inspectionId: string; questionId: string } | { kind: "finding"; findingId: string };

/**
 * Fotografías: en el celular "Tomar foto" abre la cámara trasera y
 * "Galería" permite elegir existentes. Se comprimen antes de subir.
 */
export function PhotoUploader({
  ...
  async function remove(id: string) {
    setRemoving(id);
    const result = await deleteEvidenceAction(id, inspectionId);
    setRemoving(null);
    ...
    <div>
      ...
      <div className="mt-2 grid grid-cols-2 gap-2 sm:flex">
        ...
        <button
          ...
          className="flex h-12 items-center justify-center gap-2 rounded-lg border border-border-strong bg-surface px-4 text-sm font-medium hover:bg-surface-muted"
        >
          <ImagePlus className="h-5 w-5" aria-hidden /> Galería
        </button>
      </div>
      <input
        ref={cameraRef}
